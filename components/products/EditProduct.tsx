...
export default function EditProduct({
	...
	const {
		...
		defaultValues: {
			name: product.name,
			currentPricePerPackage: product.currentPricePerPackage,
	...
	const onSubmit = async (data: ProductFormValues) => {
		...
		if (!Number.isFinite(parsedPrice) || parsedPrice <= 0) {
			setError("currentPricePerPackage", {
				message: t("productEdit.pricePositive"),
			});
			return;
		}
		...
		try {
			const updated = await updateProduct({
				id: product.id,
				name: data.name,
				currentPricePerPackage: parsedPrice.toString(),
	...
	return (
		...
		>
			...
			>
				{isLoading ? t("common.saving") : t("productEdit.save")}
			</button>
		</form>
	);
}
