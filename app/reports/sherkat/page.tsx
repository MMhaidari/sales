...
export default function SherkatReportPage() {
	const { t } = useLanguage();
			...
			<div className="flex flex-wrap items-center justify-between gap-4">
				<div>
					...
					<h1 className="mt-2 text-3xl font-semibold text-slate-900">
						{t("sherkatReport.title")}
					</h1>
					<p className="mt-1 text-sm text-slate-600">
						{t("sherkatReport.subtitle")}
					</p>
				</div>
				<div className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700">
					{t("sherkatReport.allTime")}
				</div>
			</div>
			...
			<div className="grid gap-4 md:grid-cols-4">
				<div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
					<p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">
						{t("sherkatReport.totalBills")}
					</p>
					<p className="mt-2 text-2xl font-semibold text-slate-900">
						{sherkatStats.totalBills.toLocaleString()}
					</p>
				</div>
				<div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
					<p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">
			...
			<div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl">
				<div className="flex items-center justify-between">
					<h2 className="text-lg font-semibold text-slate-900">
						{t("sherkatReport.tableTitle")}
					</h2>
					{(billsLoading || productsLoading) && (
						<span className="text-xs text-slate-400">{t("sherkatReport.loading")}</span>
					)}
				</div>

				<div className="mt-4 space-y-3">
					{(billsLoading || productsLoading) && (
						<p className="text-sm text-slate-500">{t("sherkatReport.loading")}</p>
					)}
					{!billsLoading && !productsLoading && reportRows.length === 0 && (
						<div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-6 py-10 text-center text-sm text-slate-500">
							{t("sherkatReport.noBills")}
						</div>
					)}
					{!billsLoading && !productsLoading && reportRows.length > 0 && (
