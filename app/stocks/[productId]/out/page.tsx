...
export default function StockHistoryOutPage() {
    const { t } = useLanguage();
    ...
    const entries = stockHistory
        ? stockHistory.history.filter((entry) => entry.movementType === "OUT")
        : [];
        ...
        <section className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600">{t("stockHistory.outgoingTitle")}</p>
                </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl">
                <div className="flex items-center justify-between">
                    <h2 className="text-lg font-semibold text-slate-900">{t("stockHistory.outHistory")}</h2>
                    {stockHistory && (
                ...
                <div className="mt-4 space-y-3">
                    ...
                    {productId && !isLoading && !isError && entries.length === 0 && (
                        <p className="text-sm text-slate-500">{t("stockHistory.noOutgoing")}</p>
                    )}
                    {productId && !isLoading && !isError && entries.length > 0 && (
